...
export async function verifyAdmin(req, res, next) {
  try {
    ...
    if (idToken.includes('.') && !idToken.startsWith('admin_')) {
      try {
    ...
    if (!uid) {
      ...
      if (!idToken.includes('.') || idToken.startsWith('admin_') || idToken.includes('_')) {
        uid = idToken;
      } else {
    ...
    if (!userSnapshot.exists) {
      // If auth passes but document doesn't exist yet, we can check custom claims,
      // or for local testing/bootstrapping, let's fallback to checking if it's the admin uid.
      // But we will stick to the Firestore check first.
